...
import com.rmas.wildriff.databinding.FragmentHomeFilterBinding
...
class HomeFilterFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        myRiffsViewModel.fetchUserRiffs(sharedViewModel.userId.value!!) { success, error ->
            if (success) {
                Toast.makeText(requireContext(), "Fetching success!!!", Toast.LENGTH_SHORT)
                    .show()
            } else {
                Toast.makeText(requireContext(), "Failed with $error", Toast.LENGTH_SHORT)
                    .show()
            }
        ...
        binding.buttonApplyFilter.setOnClickListener {
            ...
            findNavController().navigate(R.id.action_HomeFilterFragment_to_HomeFragment, args)
        }
